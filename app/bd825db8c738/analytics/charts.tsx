...
export function DonutChart({
  ...
  if (total === 0) {
    return <div className="flex items-center justify-center text-xs" style={{ height: 96, color: "var(--admin-text-faint)" }}>No opens in this window.</div>;
  }

  return (
    <div className="flex items-center gap-6">
      <svg viewBox="0 0 100 100" width={110} height={110} style={{ overflow: "visible" }}>
        <circle cx={50} cy={50} r={R} fill="none" stroke="var(--admin-border)" strokeWidth={14} />
        {arcs.map((a, i) => (
          <circle
            key={a.label}
      ...
      <ul className="flex flex-col gap-2.5">
        {arcs.map((a, i) => (
          ...
          >
            ...
            <span style={{ color: "var(--admin-text-secondary)" }}>{a.label}</span>
            <span className="font-bold" style={{ color: "var(--admin-text)" }}>{total ? Math.round((a.value / total) * 100) : 0}%</span>
          </li>
        ))}
      </ul>
    </div>
